    ...
    <footer className="relative border-t border-white/5 bg-black">
      <div className="mx-auto max-w-[1440px] px-4 py-7 md:px-8 md:py-10">
        <div className="flex flex-col items-center justify-between gap-7 md:flex-row md:gap-8">
          ...
          <nav className="flex max-w-[min(100%,42rem)] flex-wrap justify-center gap-x-5 gap-y-2.5 md:gap-x-7 md:gap-y-3" aria-label={t.footer.navAria}>
            {footerLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-[12px] font-medium uppercase tracking-[0.14em] text-foreground-muted transition-colors hover:text-white md:text-[13px]"
              >
                {link.label}
              </a>
            ))}
